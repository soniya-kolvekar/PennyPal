...
export default function GoalDetailPage({ params: paramsPromise }) {
  ...
  const handleAddSavings = async (amount) => {
    ...
    try {
      ...
      setToastMessage(`Added ₹${num.toLocaleString()} to ${goal.title}!`);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#FAF9FF] to-[#EAE3FA]/50 rounded-3xl p-6 border border-[#EAE3FA] shadow-sm text-center">
              ...
              <div className="inline-flex items-center gap-1 text-xs font-bold text-[#5B3F91] bg-white px-3 py-1 rounded-full border border-[#EAE3FA] mb-2">
                <Image src="/heart.png" alt="Heart" width={14} height={14} className="inline" />
                <span>Penny's Tip</span>
              </div>
              <p className="text-xs text-[#5B3F91] font-medium leading-relaxed">
                "You're doing fantastic on your <span className="font-bold">{goal.title}</span> quest!
                Every small deposit brings you closer to your dream."
              </p>
            </div>
          </div>
        </div>
